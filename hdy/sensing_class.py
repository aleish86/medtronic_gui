...
class sensing(object):
    ...
    def fix_lag(self):
        #     Correlate ECGs
        # try:
        #     corr = scipy.signal.correlate(self.resampled_shock_data, self.resampled_bipecg_data, mode="full",
        #                                   method="fft")
        #     lags = scipy.signal.correlation_lags(len(resampled_shock_data), len(self.resampled_bipecg_data),
        #                                      mode="full")
        # except:
        #     corr = scipy.signal.correlate(self.resampled_rv_data, self.resampled_bipecg_data, mode="full",
        #                                   method="fft")
        #     lags = scipy.signal.correlation_lags(len(resampled_rv_data), len(self.resampled_bipecg_data),
        #                                      mode="full")

        corr = scipy.signal.correlate(self.resampled_rv_data[0:6000], self.resampled_bipecg_data[0:6000], mode="full",
                                      method="fft")
        lags = scipy.signal.correlation_lags(len(self.resampled_rv_data), len(self.resampled_bipecg_data),
                                             mode="full")
        ...
        if lag < 0:
            ...
            self.rvbip_data = np.delete(self.resampled_rv_data, remove, axis=0)
            self.rvshock_data = np.delete(self.resampled_shock_data, remove, axis=0)
            self.ra_data = np.delete(self.resampled_ra_data, remove, axis=0)
            self.lv_data = np.delete(self.resampled_lv_data, remove, axis=0)

            self.ecg3_data = self.resampled_ecg3_data[0:len(self.rvbip_data)]
            self.bipecg_data= self.resampled_bipecg_data[0:len(self.rvbip_data)]
            self.laser1_data = self.resampled_laser1_data[0:len(self.rvbip_data)]
            self.laser2_data = self.resampled_laser1_data[0:len(self.rvbip_data)]
            self.bp_data = self.resampled_bp_data[0:len(self.rvbip_data)]

    def align_peaks(self, egm, ecg):
        diff = ecg - egm
        if diff > 0:
            egm = np.delete(egm, diff)
            self.filt_rvbip_peaks = np.delete(self.filt_rvbip_peaks, diff)
            self.filt_rvshock_peaks = np.delete(self.filt_rvshock_peaks, diff)
            self.filt_ralead_peaks = np.delete(self.filt_ralead_peaks, diff)

        else:
            ecg = np.delete(ecg, diff)
        getattr(self, 'filt_' + item + '_peaks')


    def resample_data(self, original_fs, desired_fs):
        sec = (self.laser_exp.bipecg.data.size) / original_fs
        ...
        self.resampled_bipecg_data = scipy.signal.resample(self.laser_exp.bipecg.data, new_length)
        ...
        # resampled_bipecg_data = scipy.signal.resample(self.laser_exp.bipecg.data, new_length)
        ...
        # log_bipecg_data = np.log(resampled_bipecg_data+10)
        ...
        # self.resampled_bipecg_data = scaler.fit_transform(log_bipecg_data.reshape(-1, 1)).reshape(-1)
    ...
    def ecg_filter(self):
        ...
        self.bipecg_data = scipy.signal.sosfilt(ecg_sos, self.resampled_bipecg_data)
    ...
    def amplifier(self):
        ...
        self.bipecg_data = self.bipecg_data * 3
        self.ecg3_data = self.ecg3_data * 3

    def rectifier(self):
        # Rectifying the ECG
        self.rect_ecg3 = abs(self.ecg3_data)
        self.rect_bipecg = abs(self.bipecg_data)
    ...
    def peaks_zero_crossings(self):
        self.zero_crossings()
        items = ['bipecg', 'ecg3', 'ralead', 'rvbip', 'rvshock', 'lvlead', 'laser1', 'laser2']
        for item in items:
            try:
                prev_crossing = None
