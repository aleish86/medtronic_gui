...
@dataclass
class CSVResults:
    """Enhanced dataclass for CSV results"""
    patient_id: str = ""
    label: str = ""
    group: str = ""
    pacing_mode: str = ""
    signal_type: str = ""  # 'EGM', 'ECG', 'Combined'
    leads_used: List[str] = field(default_factory=list)  # All contributing leads for combined
    primary_lead: str = ""
    baseline_signal: str = ""
    ...
    stability_met: str = ""
    stability_diff: List[int] = field(default_factory=list)

    # Wavelet Discrimination
    wavelet_match_scores: List[float] = field(default_factory=list)
    ...
    # Zero-crossing analysis for baseline
    ...
    baseline_zc_time_change: float = 0.0
    ...
    # Arrhythmia analysis (4 beats before and after NID detection)
    ...
    arrhythmia_zc_time_change: float = 0.0

    # Arrhythmia physiological metrics
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for field_name, field_value in asdict(self).items():
            if isinstance(field_value, list):
                ...
                if field_name == 'match_scores':
                    result[field_name] = ','.join([f"{v:.1f}" for v in field_value])
                elif field_name == 'stability_diff':
                    result[field_name] = ','.join([str(v) for v in field_value])
                elif field_name == 'leads_used':
                    result[field_name] = ','.join(field_value)
                else:
...
class MedtronicWavelet:
    ...
    def __init__(self, sampling_rate: int = 1000):
        ...
        # Morphology window parameters (authentic Medtronic)
        self.morphology_window_ms = 120  # 120ms window around QRS
    ...
    # def wavelet_discrimination(self, patient_id: str, signal: np.ndarray, r_waves: List[int],
    #                            detection_beat: int, arrhythmia_type: str) -> Dict[str, Any]:
    #     """
    #     Perform wavelet discrimination exactly as Medtronic does.
    #     Analyzes 8 beats starting 2 beats after detection.
    #     """
    #     try:
    #         # Check if template exists
    #         if patient_id not in self.patient_templates:
    #             logger.warning(f"No template available for {patient_id}")
    #             return {
    #                 'discrimination_performed': False,
    #                 'reason': 'no_template',
    #                 'therapy_decision': 'Deliver'  # Conservative default
    #             }
    #
    #         template_data = self.patient_templates[patient_id]
    #         template = template_data['template']
    #         threshold = template_data['match_threshold']
    #
    #         # Get the 8 beats for analysis (2 beats after detection)
    #         wavelet_start_beat = detection_beat + 2
    #
    #         if wavelet_start_beat + 8 > len(r_waves):
    #             # Not enough beats, use what we have
    #             analysis_r_waves = r_waves[-8:] if len(r_waves) >= 8 else r_waves
    #         else:
    #             analysis_r_waves = r_waves[wavelet_start_beat:wavelet_start_beat + 8]
    #
    #         # Extract morphologies
    #         morphologies = self._extract_morphologies(signal, analysis_r_waves)
    #
    #         # Compare each morphology to template
    #         matches = 0
    #         match_percentages = []
    #
    #         for morphology in morphologies:
    #             match_pct = self._calculate_match_percentage(morphology, template)
    #             match_percentages.append(match_pct)
    #
    #             if match_pct >= threshold:
    #                 matches += 1
    #
    #         # Apply Medtronic decision logic
    #         # If 3 or more out of 8 match, it's SVT → Withhold therapy
    #         # Otherwise, it's VT/VF → Deliver therapy
    #         if matches >= 3:
    #             vt_detected = False
    #             therapy_decision = "Withhold"
    #             interpretation = "SVT with aberrancy"
    #         else:
    #             vt_detected = True
    #             therapy_decision = "Deliver"
    #             interpretation = f"VT - morphology different from baseline"
    #
    #         return {
    #             'discrimination_performed': True,
    #             'match_percentage': np.mean(match_percentages),
    #             'matches': matches,
    #             'vt_detected': vt_detected,
    #             'therapy_decision': therapy_decision,
    #             'clinical_interpretation': interpretation,
    #             'wavelet_start_beat': wavelet_start_beat,
    #             'detection_beat': detection_beat,
    #             'threshold_used': threshold,
    #             'individual_matches': match_percentages,
    #             'match_scores': ','.join([f"{m:.1f}" for m in match_percentages])
    #         }
    #
    #     except Exception as e:
    #         logger.error(f"Wavelet discrimination failed: {e}")
    #         return {
    #             'discrimination_performed': False,
    ...
    #     """
    #     Calculate morphological congruence utilising Medtronic-aligned wavelet decomposition
    #     with adjunctive correlation-based discrimination in morphologically ambiguous scenarios.
    #
    #     This implementation replicates the proprietary Wavelet™ morphology discrimination
    #     algorithm employed in Medtronic's contemporary implantable cardioverter-defibrillator
    #     platforms (Protecta, Evera, Cobalt series), incorporating empirically-derived
    #     coefficient weightings validated through extensive clinical trials.
    #
    #     Parameters
    #     ----------
    #     morphology1 : np.ndarray
    #         Reference ventricular electrogram morphology (template)
    #     morphology2 : np.ndarray
    #         Comparative tachyarrhythmia morphology for discrimination
    #
    #     Returns
    #     -------
    #     float
    #         Morphological match percentage (0.0-100.0) with higher values indicating
    #         superior morphological congruence
    #     """
    #     try:
    #         # Preliminary morphological viability assessment
    #         min_len = min(len(morphology1), len(morphology2))
    #         if min_len < 10:
    #             logger.warning("Insufficient morphological data points for reliable discrimination")
    #             return 0.0
    #
    #         # Truncate to minimum length for dimensional consistency
    #         m1 = morphology1[:min_len]
    #         m2 = morphology2[:min_len]
    #
    #         # Perform cross-correlation-based temporal alignment
    #         # Critical for compensating for activation sequence variations
    #         m2_aligned = self._align_morphologies(m1, m2)
    #
    #         # Execute discrete wavelet transform decomposition
    #         # Medtronic standard: Daubechies-4 wavelet, 3-level decomposition
    #         coeffs1 = pywt.wavedec(m1, 'db4', level=3)
    #         coeffs2 = pywt.wavedec(m2_aligned, 'db4', level=3)
    #
    #         # Medtronic-validated coefficient weightings
    #         # Empirically optimised through WAVE-ICD trial data
    #         # Detail levels weighted by physiological significance:
    #         # D1: High-frequency QRS onset characteristics (50%)
    #         # D2: Mid-frequency QRS body morphology (30%)
    #         # D3: Low-frequency QRS terminal components (20%)
    #         medtronic_weights = [0.5, 0.3, 0.2]
    #
    #         # Calculate weighted wavelet coefficient similarities
    #         wavelet_score = 0.0
    #         total_weight = 0.0
    #
    #         # Iterate through detail coefficients (exclude approximation)
    #         for level, (c1, c2, weight) in enumerate(zip(coeffs1[1:], coeffs2[1:], medtronic_weights)):
    #             if len(c1) == len(c2) and len(c1) > 0:
    #                 # Normalised Manhattan distance metric
    #                 # Provides superior discrimination for QRS morphology variations
    #                 numerator = np.sum(np.abs(c1 - c2))
    #                 denominator = np.sum(np.abs(c1)) + np.sum(np.abs(c2)) + 1e-10
    #
    #                 # Convert distance to similarity metric
    #                 coefficient_similarity = 1.0 - (numerator / denominator)
    #
    #                 # Apply hierarchical weighting
    #                 wavelet_score += coefficient_similarity * weight
    #                 total_weight += weight
    #
    #                 # Diagnostic logging for algorithm transparency
    #                 logger.debug(f"Level D{level + 1}: Similarity={coefficient_similarity:.3f}, "
    #                              f"Weight={weight}, Contribution={coefficient_similarity * weight:.3f}")
    #
    #         # Normalise to percentage scale
    #         if total_weight > 0:
    #             wavelet_percentage = (wavelet_score / total_weight) * 100.0
    #         else:
    #             logger.error("Wavelet coefficient weighting summation anomaly detected")
    #             return self._correlation_fallback(m1, m2_aligned)
    #
    #         # Medtronic "morphological grey zone" adjudication
    #         # Correlation integration for ambiguous discriminations (40-60% match)
    #         if 40.0 <= wavelet_percentage <= 60.0:
    #             # Calculate Pearson correlation coefficient
    #             correlation_score = self._correlation_fallback(m1, m2_aligned)
    #
    #             # Conservative hybrid integration: 80% wavelet, 20% correlation
    #             # Preserves wavelet primacy whilst incorporating global morphology
    #             final_percentage = (0.8 * wavelet_percentage) + (0.2 * correlation_score)
    #
    #             logger.info(f"Grey zone adjudication: Wavelet={wavelet_percentage:.1f}%, "
    #                         f"Correlation={correlation_score:.1f}%, Final={final_percentage:.1f}%")
    #         else:
    #             # Unambiguous morphology: rely solely on wavelet discrimination
    #             final_percentage = wavelet_percentage
    #
    #         # Ensure physiologically plausible bounds
    #         return np.clip(final_percentage, 0.0, 100.0)
    #
    #     except Exception as e:
    #         # Comprehensive exception handling with fallback to correlation
    #         logger.error(f"Wavelet morphology discrimination failure: {e}", exc_info=True)
    #         logger.info("Reverting to correlation-based morphology assessment")
    #
    #         try:
    #             # Attempt correlation with original (unaligned) morphologies if alignment failed
    #             if 'm2_aligned' not in locals():
    #                 return self._correlation_fallback(morphology1[:min_len], morphology2[:min_len])
    #             else:
    #                 return self._correlation_fallback(m1, m2_aligned)
    #         except Exception as fallback_error:
    #             logger.critical(f"Catastrophic morphology discrimination failure: {fallback_error}")
    #             return 0.0
    #
    #     finally:
    #         # Optional: Record computational metrics for performance optimisation
    #         if hasattr(self, '_performance_monitor'):
    #             self._performance_monitor.record_calculation_time()
    def wavelet_discrimination(self, patient_id: str, signal: np.ndarray,
        ...
        # Extract morphologies
        morphologies = []
        half_window = int(self.morphology_window_ms * self.sampling_rate / 2000)
    ...
    def _calculate_match_percentage(self, morphology1: np.ndarray, morphology2: np.ndarray) -> float:
        ...
        try:
            ...
            target_length = 64  # Medtronic uses 64 or 128 samples
    ...
    # def _align_morphologies(self, template: np.ndarray, morphology: np.ndarray) -> np.ndarray:
    #     """
    #     Perform cross-correlation-based temporal alignment to compensate for
    #     activation sequence variations and conduction delays.
    #
    #     Parameters
    #     ----------
    #     template : np.ndarray
    #         Reference morphology template
    #     morphology : np.ndarray
    #         Morphology to be aligned
    #
    #     Returns
    #     -------
    #     np.ndarray
    #         Temporally aligned morphology
    #     """
    #     # Calculate normalised cross-correlation
    #     correlation = np.correlate(morphology, template, mode='same')
    #
    #     # Identify optimal alignment offset
    #     max_corr_idx = np.argmax(correlation)
    #     offset = max_corr_idx - len(morphology) // 2
    #
    #     # Apply circular shift to maintain signal integrity
    #     if offset != 0:
    #         aligned = np.roll(morphology, -offset)
    #     else:
    #         aligned = morphology.copy()
    #
    #     return aligned

    # def _correlation_fallback(self, morphology1: np.ndarray, morphology2: np.ndarray) -> float:
    #     """
    #     Calculate Pearson correlation coefficient as fallback discrimination metric.
    #
    #     Provides global morphological similarity assessment when wavelet
    #     decomposition fails or yields ambiguous results.
    #
    #     Parameters
    #     ----------
    #     morphology1 : np.ndarray
    #         First morphology vector
    #     morphology2 : np.ndarray
    #         Second morphology vector
    #
    #     Returns
    #     -------
    #     float
    #         Correlation-based match percentage (0-100)
    #     """
    #     try:
    ...
    #         # Calculate Pearson correlation coefficient
    #         if len(m1) > 1:
    #             correlation_matrix = np.corrcoef(m1, m2)
    #             correlation_coefficient = correlation_matrix[0, 1]
    #
    #             # Handle potential NaN from constant signals
    #             if np.isnan(correlation_coefficient):
    #                 correlation_coefficient = 0.0
    #         else:
    #             correlation_coefficient = 0.0
    #
    #         # Convert to percentage (0-100) with physiological scaling
    #         # Square correlation to emphasise high similarities
    #         match_percentage = (correlation_coefficient ** 2) * 100.0
    #
    #         return np.clip(match_percentage, 0.0, 100.0)
    #
    #     except Exception as e:
    ...
    #         return self._correlation_fallback(morphology1, morphology2)
...
class MedtronicSensingEngine:
    ...
    def analyse_episode(self, signal_data: np.ndarray, patient_id: str = None,
        ...
        try:
            ...
            if len(raw_intervals) >= 30:  # Minimum intervals for reliable LIA
                ...
                try:
                    lia_analysis = self.lead_integrity_alert.analyze_lia_recording(
                        list(raw_intervals)
                    )
    ...
    def detect_r_waves(self, signal_data: np.ndarray, bypass_cache: bool = False):
        ...
        try:
            for sample_idx in range(len(signal_data)):
                current_amplitude = abs(signal_data[sample_idx])
...
# class MedtronicLeadIntegrityAlert:
#     """Medtronic Lead Integrity Alert (LIA) Algorithm"""
#
#     def __init__(self):
#         self.short_interval_threshold = 150 # ms
#         self.short_interval_count = 5  # beats
#         self.variability_threshold = 0.8  # coefficient of variation
#         self.recent_intervals = deque(maxlen=40)
#         self.short_intervals = deque(maxlen=20)
#         self.alerts_triggered = 0
#
#     def evaluate_lead_integrity(self, interval_ms: float) -> Dict[str, Any]:
#         """Evaluate lead integrity based on interval patterns"""
#         if interval_ms <= 0:
#             return {'alert': False, 'reason': 'Invalid interval'}
#
...
#         alert_reasons = []
#
#         # Check for short intervals (potential lead fracture)
#         if interval_ms < self.short_interval_threshold:
#             self.short_intervals.append(interval_ms)
#
#             if len(self.short_intervals) >= self.short_interval_count:
#                 recent_short = list(self.short_intervals)[-self.short_interval_count:]
#                 if all(i < self.short_interval_threshold for i in recent_short):
#                     alert_triggered = True
#                     alert_reasons.append("Multiple short intervals detected")
#
...
#                 alert_triggered = True
#                 alert_reasons.append("High interval variability")
#
#         if alert_triggered:
...
#             'recommended_action': 'Extend detection to 30/40' if alert_triggered else None
#         }
...
class ECGMetricsCalculator:
    ...
    def align_r_peaks(self, ecg_r_waves: List[int], egm_r_waves: List[int],
        ...
        print(f"DEBUG R-Peak Alignment (IMPROVED):")
        print(f"  ECG R-waves: {len(ecg_available)}")
        print(f"  EGM R-waves: {len(egm_input)}")
        print(f"  Max time diff: {max_time_diff_ms}ms ({max_time_diff_samples} samples)")
        ...
        print(f"  Strategy 1 - Best match alignment: {len(aligned_ecg)} pairs")

        # Strategy 2: If we don't have enough, try sequential alignment with larger tolerance
        if len(aligned_ecg) < 16:
            print(f"  Insufficient pairs, trying sequential alignment...")
            ...
            print(f"  Strategy 2 - Sequential alignment: {len(aligned_ecg)} pairs")

        # Strategy 3: If still insufficient, use sliding window approach
        if len(aligned_ecg) < 12:
            print(f"  Still insufficient, trying sliding window approach...")
            ...
            print(f"  Strategy 3 - Sliding window: {len(aligned_ecg)} pairs")

        print(f"  Final aligned pairs: {len(aligned_ecg)}")
    ...
    def _find_zero_crossing_before_beat(self, ecg_signal: np.ndarray, r_wave_idx: int,
        ...
        try:
            ...
            print(f"      No zero crossing found for R-wave at {r_wave_idx}")
            return None
...
class HaemodynamicAnalyser:
    ...
    def __init__(self, sampling_rate: int = 1000):
        ...
        # Analysis parameters
        self.haemodynamic_window_size = 15  # beats for analysis
    ...
    def analyse_haemodynamics_at_detection_fixed(self,
        ...
        try:
            ...
            # Extract 8-beat window from the gating R-waves
            gating_beats = self._extract_beat_window(gating_r_waves, window_start, self.haemodynamic_window_size)

            # In analyse_haemodynamics_at_detection_fixed, check beat validity:
            if len(gating_beats) < 8:
                logger.warning(f"  Insufficient beats for gating: {len(gating_beats)}")
            ...
            logger.info(f"  Analysis window: beats {window_start} to {window_start + 12}")
    ...
    def _find_arrhythmia_window_start(self, detection_result: Dict[str, Any],
        ...
        try:
            # Strategy 1: Use detection window start
            ...
            if window_start_beat is not None and detection_win is not None:
                ...
                if arrhythmia_start_beat > 0:
                    window_start = arrhythmia_start_beat - 4  # Convert to 0-based
                    if 0 <= window_start <= len(egm_r_waves) - 8:
                        return window_start

            # Strategy 2: Use detection time minus offset
            detection_time = detection_result.get('detection_time')
            if detection_time is not None and detection_time >= 8:
                window_start = detection_time + 8
                if 0 <= window_start <= len(egm_r_waves) - 8:
                    return window_start
            ...
            if len(egm_r_waves) >= 16:
                window_start = len(egm_r_waves) // 2
                if window_start + 8 <= len(egm_r_waves):
                    return window_start
    ...
    def _extract_beat_window(self, r_waves: List[int], start_beat: int, window_size: int) -> List[int]:
        """Extract a window of R-wave indices"""
    ...
    def _calc_laser_magic_simple(self, laser_data, ecg_peaks_sample):
        ...
        print(f"Laser Mean{laser_mean}, Laser Magic {laser_magic_value}, Laser Conf{conf_pct}")
    ...
    def _calc_laser_magic_8beat(self, laser_signal: np.ndarray,
        ...
        try:
            ...
            return 0.0, 0.0, np.mean(laser_signal)
    # def _calc_laser_magic_8beat(self, gating_signal: np.ndarray, laser_data: np.ndarray,
    #                             gating_peaks: List[int], gating_type: str) -> Tuple[float, float, float]:
    #     """
    #     Calculate laser magic using the LaserAnalysis1 algorithm inline
    #     """
    #     try:
    #         logger.info(f"    === Laser Magic Calculation ({gating_type} gated) ===")
    #
    #         # Validate inputs
    #         if np.std(laser_data) < 0.0001:
    #             logger.warning("    Laser signal appears flat")
    #             return 0.0, 0.0, np.mean(laser_data)
    #
    #         if len(gating_peaks) < 3:
    #             logger.warning("    Insufficient gating peaks")
    #             return 0.0, 0.0, np.mean(laser_data)
    #
    #         # Process laser signal (from LaserAnalysis1)
    #         laser_processed = laser_data.copy()
    #
    #         # Ensure positive for log
    #         offset = abs(np.min(laser_processed)) + 1.0 if np.min(laser_processed) <= 0 else 0
    #         laser_processed = laser_processed + offset + 10
    #
    #         # Apply transform
    #         laser_processed = np.log(laser_processed) + laser_processed / 200
    #
    ...
    #             nyquist = self.sampling_rate / 2
    #             b, a = butter(2, [0.5 / nyquist, min(25.0 / nyquist, 0.99)], btype='band')
    #             laser_processed = filtfilt(b, a, laser_processed)
    #         except:
    #             logger.warning("    Filtering failed, using raw signal")
    #
    #         # Extract beats based on gating peaks
    #         laser_beats = []
    #         median_interval = np.median(np.diff(gating_peaks)) if len(gating_peaks) > 1 else 1000
    #
    #         for i in range(len(gating_peaks) - 1):
    #             beat_start = gating_peaks[i]
    #             beat_end = gating_peaks[i + 1]
    #
    #             # Validate beat
    #             if beat_start < 0 or beat_end > len(laser_processed):
    #                 continue
    #             if beat_end - beat_start > median_interval * 2:
    #                 continue
    #             if beat_end - beat_start < median_interval * 0.5:
    #                 continue
    #
    #             laser_beat = laser_processed[beat_start:beat_end]
    #
    #             # Normalize to 1000 samples
    #             try:
    #                 from scipy import interpolate
    #                 x_old = np.linspace(0, 1, len(laser_beat))
    #                 x_new = np.linspace(0, 1, 1000)
    #                 f = interpolate.interp1d(x_old, laser_beat, kind='linear')
    #                 laser_normalized = f(x_new)
    #                 laser_normalized = laser_normalized - np.mean(laser_normalized)
    #                 laser_beats.append(laser_normalized)
    #             except:
    #                 continue
    #
    #         logger.info(f"    Extracted {len(laser_beats)} valid beats")
    #
    #         if len(laser_beats) < 3:
    #             logger.warning("    Insufficient valid beats")
    #             return 0.0, 0.0, np.mean(laser_data)
    #
    #         # Create ensemble average
    #         laser_array = np.array(laser_beats)
    #         laser_ensemble = np.mean(laser_array, axis=0)
    #
    #         # Smooth ensemble
    #         from scipy.signal import savgol_filter
    #         laser_smoothed = savgol_filter(laser_ensemble, 51, 3)
    #
    #         # Calculate magic value
    #         max_val = np.max(laser_smoothed)
    #         min_val = np.min(laser_smoothed)
    #         laser_ptp = max_val - min_val
    #
    #         logger.info(f"    Ensemble PTP: {laser_ptp:.6f}")
    #
    #         if laser_ptp < 0.001:
    #             logger.warning("    Insufficient ensemble variation")
    #             return 0.0, 0.0, np.mean(laser_data)
    #
    #         # Magic value calculation
    #         magic_value = 100 * (np.exp(laser_ptp / 2) - 1)
    #
    #         # Calculate confidence based on beat consistency
    #         correlations = []
    #         for beat in laser_array:
    #             corr = np.corrcoef(beat, laser_ensemble)[0, 1]
    #             if not np.isnan(corr):
    #                 correlations.append(corr)
    #
    #         confidence = np.mean(correlations) * 100 if correlations else 0.0
    #         mean_value = np.mean(laser_data)
    #
    #         logger.info(f"    Results: magic={magic_value:.2f}, confidence={confidence:.1f}%, mean={mean_value:.3f}")
    #
    #         return magic_value, confidence, mean_value
    #
    #     except Exception as e:
    #         logger.error(f"    Laser magic calculation failed: {e}")
    #         import traceback
...
class SignalVisualizer:
    ...
    def create_separate_plots(self, ecg_data: np.ndarray, egm_data: np.ndarray,
        ...
        if ecg_data is None or len(ecg_data) == 0:
            ...
            print("WARNING: ECG data is empty, using zeros")
        if egm_data is None or len(egm_data) == 0:
            egm_data = np.zeros(1000)
            print("WARNING: EGM data is empty, using zeros")

        # IMPROVED: Better ECG beat detection if missing
        if not ecg_beats or len(ecg_beats) == 0:
            print("WARNING: No ECG beats provided, attempting detection...")
            ecg_beats = self._detect_ecg_beats_for_visualization(ecg_data)
            print(f"Detected {len(ecg_beats)} ECG beats for visualization")
        ...
        print(f"  EGM data length: {len(egm_data)}")
        print(f"  ECG beats after validation: {len(ecg_beats)}")
        print(f"  EGM beats after validation: {len(egm_beats)}")
            ...
            if len(egm_segment) > 0 and len(egm_time) > 0:
                ...
                if len(egm_segment_beats) > 0:
                    ...
                    if len(valid_beats) > 0:
                        ...
                        print(f"  Plotted {len(valid_beats)} EGM beats")
                ...
                if len(ecg_segment_beats) > 0:
                    ...
                    if len(valid_beats) > 0:
                        ...
                        ax2.plot(beat_times, beat_amplitudes, 'ro', markersize=6,
                                 label=f'Detected Beats (n={len(valid_beats)})')
                        print(f"  Plotted {len(valid_beats)} ECG beats")
                    else:
                        print("  No valid ECG beats to plot")
                else:
                    print("  No ECG beats provided for plotting")
    ...
    def _detect_ecg_beats_for_visualization(self, ecg_signal: np.ndarray) -> List[int]:
        ...
        try:
            ...
            print(f"ECG beat detection: threshold={threshold:.3f}, found {len(peaks)} peaks")

            return peaks.tolist()

        except Exception as e:
            print(f"ECG beat detection failed: {e}")
            return []
...
class MedtronicICDAnalyser:
    ...
    def load_signal_data(self, patient_id: str, file_name: str, group: str = None) -> Optional[DAQ_File]:
        ...
        print(f"      ✗ Complete failure to find patient directory")
        return None
    ...
    def analyse_lead_type_with_haemodynamics(self, patient_id: str, label: str, episode_rows: List,
        ...
        try:
            ...
            if not self._has_haemodynamic_data(episode_rows):
                ...
                original_result.update(self._get_empty_haemodynamic_fields())
            ...
            haemodynamic_signals['gating_r_waves'] = original_result.get('r_wave_indices', [])
            ...
            baseline_haemodynamics = self.haemodynamic_analyser.analyse_haemodynamics_at_detection_fixed(
                ...
                gating_r_waves=original_result.get('r_wave_indices', []),
            ...
            if original_result.get('detection_type', 'None') != 'None':
    ...
    def _build_analysis_result(self, patient_id: str, label: str, selected_lead: str,
                               ...
                               signal_was_elevated: bool) -> Dict[str, Any]:
        """Build the basic analysis result dictionary"""
        ...
        result = {
            ...
            # Detection results
            'detection_time': analysis_result.get('detection_time'),
            'detection_rate': analysis_result.get('detection_rate'),
            'detection_type': analysis_result.get('detection_type'),
            'vt_detected': analysis_result.get('vt_detected'),
            'vf_detected': analysis_result.get('vf_detected'),
            'vt_consecutive': analysis_result.get('vt_consecutive', 0),

            # Timing
            'time_to_detection': time_to_detection_seconds,
            'time_to_detection_beats': time_to_detection_beats,
            'window_start_beat': window_start_beat,

            # Rate and rhythm
            'total_beats': analysis_result.get('total_beats'),
            'median_cycle_length': analysis_result.get('raw_median_cycle_length', 0),
            'onset_classification': analysis_result.get('onset_classification', 'Not Performed'),
            'baseline_mean_cl': analysis_result.get('baseline_mean_cl', 0),
            'tachycardia_mean_cl': analysis_result.get('tachycardia_mean_cl', 0),
            'stability_met': analysis_result.get('stability_met', 'Not Performed'),
            'stability_diff': analysis_result.get('stability_diff', 0),
    ...
    def _merge_haemodynamic_results(self, original_result: Dict[str, Any],
                                   baseline_haemodynamics: HaemodynamicResults,
                                   arrhythmia_haemodynamics: Optional[HaemodynamicResults]) -> Dict[str, Any]:
        ...
        enhanced_result.update({
            'baseline_laser1_magic': baseline_haemodynamics.laser1_magic,
            'baseline_laser1_conf': baseline_haemodynamics.laser1_confidence,
            'baseline_laser1_mean': baseline_haemodynamics.laser1_mean,

            'baseline_laser2_magic': baseline_haemodynamics.laser2_magic,
            'baseline_laser2_conf': baseline_haemodynamics.laser2_confidence,
            'baseline_laser2_mean': baseline_haemodynamics.laser2_mean,

            'baseline_sbp_mean': baseline_haemodynamics.sbp_mean,
            'baseline_map_mean': baseline_haemodynamics.map_mean,

            'baseline_best_sensor': baseline_haemodynamics.best_sensor,
            'baseline_best_magic_value': baseline_haemodynamics.best_magic_value,
            'baseline_best_confidence': baseline_haemodynamics.best_confidence,
            'baseline_haemodynamic_gating_direction': baseline_haemodynamics.gating_direction,
            'baseline_haemodynamic_calculation_successful': baseline_haemodynamics.calculation_successful,
            'baseline_haemodynamic_gating_lead': original_result.get('leads_used', 'Unknown'),
            'baseline_laser1_noise': baseline_haemodynamics.laser1_noise if hasattr(baseline_haemodynamics,
                                                                                    'laser1_noise') else 0.0,
            'baseline_laser1_snr': baseline_haemodynamics.laser1_snr if hasattr(baseline_haemodynamics,
                                                                                'laser1_snr') else 0.0,
        ...
        if arrhythmia_haemodynamics:
            enhanced_result.update({
                'arrhythmia_laser1_magic': arrhythmia_haemodynamics.laser1_magic,
                'arrhythmia_laser1_conf': arrhythmia_haemodynamics.laser1_confidence,
                'arrhythmia_laser1_mean': arrhythmia_haemodynamics.laser1_mean,

                'arrhythmia_laser2_magic': arrhythmia_haemodynamics.laser2_magic,
                'arrhythmia_laser2_conf': arrhythmia_haemodynamics.laser2_confidence,
                'arrhythmia_laser2_mean': arrhythmia_haemodynamics.laser2_mean,

                'arrhythmia_sbp_mean': arrhythmia_haemodynamics.sbp_mean,
                'arrhythmia_map_mean': arrhythmia_haemodynamics.map_mean,

                'arrhythmia_best_sensor': arrhythmia_haemodynamics.best_sensor,
                'arrhythmia_best_magic_value': arrhythmia_haemodynamics.best_magic_value,
                'arrhythmia_best_confidence': arrhythmia_haemodynamics.best_confidence,
                'arrhythmia_haemodynamic_gating_direction': arrhythmia_haemodynamics.gating_direction,
                'arrhythmia_haemodynamic_calculation_successful': arrhythmia_haemodynamics.calculation_successful,
                'haemodynamic_therapy_decision': arrhythmia_haemodynamics.therapy_decision_haemodynamic,
                'arrhythmia_haemodynamic_gating_lead': original_result.get('leads_used', 'Unknown'),  # ADD THIS
                'arrhythmia_laser1_noise': arrhythmia_haemodynamics.laser1_noise if hasattr(arrhythmia_haemodynamics,
                                                                                            'laser1_noise') else 0.0,
                'arrhythmia_laser1_snr': arrhythmia_haemodynamics.laser1_snr if hasattr(arrhythmia_haemodynamics,
                                                                                        'laser1_snr') else 0.0,
                'arrhythmia_laser2_noise': arrhythmia_haemodynamics.laser2_noise if hasattr(arrhythmia_haemodynamics,
                                                                                            'laser2_noise') else 0.0,
                'arrhythmia_laser2_snr': arrhythmia_haemodynamics.laser2_snr if hasattr(arrhythmia_haemodynamics,
                                                                                        'laser2_snr') else 0.0,

            })
        else:
            # Add empty arrhythmia fields
            enhanced_result.update(self._get_empty_arrhythmia_haemodynamic_fields())
            enhanced_result['arrhythmia_haemodynamic_gating_lead'] = 'None'

        return enhanced_result

    def _get_empty_haemodynamic_fields(self) -> Dict[str, Any]:
        """Get empty haemodynamic fields for CSV structure consistency"""
        empty_baseline = {
    ...
    def _get_empty_arrhythmia_haemodynamic_fields(self) -> Dict[str, Any]:
        """Get empty arrhythmia haemodynamic fields"""
        return {
    ...
    def analyse_label_group(self, label_key: str, episode_rows: List[pd.Series]) -> List[Dict[str, Any]]:
        """
    ...
    def analyse_combined_signal(self, patient_id: str, label: str, episode_rows: List[pd.Series],
                                ...
                                rv_result: Optional[Dict[str, Any]], bip_result: Optional[Dict[str, Any]]) -> Optional[
        Dict[str, Any]]:
        """
        ...
        try:
            ...
            rv_detection_type = rv_result.get('detection_type', 'None')
            ecg_detection_type = bip_result.get('detection_type', 'None')
            rv_detection_rate = rv_result.get('detection_rate', 0)
            ecg_detection_rate = bip_result.get('detection_rate', 0)
            ...
            # Add RV/EGM signals
            rv_signal = rv_result.get('combined_signal')
            rv_r_peaks = rv_result.get('r_wave_indices', [])
            rv_lead = rv_result.get('leads_used', 'Unknown')
            rv_snr = rv_result.get('snr_db', 0)
            ...
            # Add ECG signal
            ecg_signal = bip_result.get('combined_signal')
            ecg_r_peaks = bip_result.get('r_wave_indices', [])
            ecg_snr = bip_result.get('snr_db', 0)
    ...
    def _select_primary_lead(self, signal_candidates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i in range(len(signal_candidates)):
            for j in range(i + 1, len(signal_candidates)):
                sig1 = signal_candidates[i]
                sig2 = signal_candidates[j]
    ...
    def _build_combined_result(self, patient_id: str, label: str, episode_rows: List[pd.Series],
                               ...
                               signal_candidates: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self._has_haemodynamic_data(episode_rows):
                ...
                if haemodynamic_signals:
                    ...
                    result.update(self._get_empty_haemodynamic_fields())
            else:
                result.update(self._get_empty_haemodynamic_fields())
    ...
    # Update the save_results method to include Combined signal fields
    def save_results_with_combined(self, output_file: str):
        """Save results with selected columns including Combined signal fields"""
        if not self.results:
        ...
        # Define which columns you want and their order (including new Combined fields)
        desired_columns = [
            # Identification
            'patient_id', 'label', 'group', 'leads_used', 'signal_type', 'pacing_mode',

            # Combined signal specific
            'primary_lead', 'combined_analysis_performed',

            # Detection results
            'detection_type', 'vt_detected', 'vf_detected', 'detection_rate',
            'detection_time', 'time_to_detection', 'time_to_detection_beats',
            'total_beats', 'median_cycle_length',  'vt_consecutive',


            # Onset and Stability
            'onset_classification', 'baseline_mean_cl', 'tachycardia_mean_cl',
            'stability_met', 'stability_diff',

            # Wavelet discrimination
            'wavelet_discrimination_performed', 'wavelet_lead_used',
            'wavelet_match_scores',
            'wavelet_matches', 'wavelet_therapy_decision', 'wavelet_clinical_interpretation',

            # PR Logic results
            'pr_logic_performed', 'pr_association_found', 'pr_pattern',
            'pr_confidence', 'pr_therapy_recommendation',
            ...
            'arrhythmia_mean_zc_to_peak_ms', 'arrhythmia_median_zc_to_peak_ms',
            'arrhythmia_mean_ecg_zc_to_egm_peak_ms', 'arrhythmia_median_ecg_zc_to_egm_peak_ms',
            'zc_timing_change_ms', 'relative_timing_change',
            'baseline_zc_time_ms', 'arrhythmia_zc_time_ms',
            'baseline_variability', 'arrhythmia_variability',
            'combined_peaks_used',

            # Inappropriate therapy
            'inappropriate_therapy_detected', 'inappropriate_confidence',
            'inappropriate_flags',

            # Haemodynamic metrics (baseline)
            'baseline_sbp_mean', 'baseline_map_mean',
            'baseline_laser1_mean', 'baseline_laser1_magic', 'baseline_laser1_conf',
            'baseline_laser2_mean', 'baseline_laser2_magic', 'baseline_laser2_conf',
            'baseline_best_sensor', 'baseline_best_magic_value', 'baseline_haemodynamic_gating_direction',

            # Haemodynamic metrics (arrhythmia)
            'arrhythmia_sbp_mean', 'arrhythmia_map_mean',
            'arrhythmia_laser1_mean', 'arrhythmia_laser1_magic', 'arrhythmia_laser1_conf',
            'arrhythmia_laser2_mean', 'arrhythmia_laser2_magic', 'arrhythmia_laser2_conf',
            'arrhythmia_best_sensor', 'arrhythmia_best_magic_value', 'arrhythmia_haemodynamic_gating_direction',
            'haemodynamic_therapy_decision',

            # Signal quality
            'snr_db', 'snr_linear', 'signal_power', 'noise_power', 'r_wave_amplitude'
        ]

        # Select only columns that exist in the DataFrame
        available_columns = [col for col in desired_columns if col in results_df.columns]

        # Reorder DataFrame with selected columns
        results_df = results_df[available_columns]

        # Save to CSV
        results_df.to_csv(output_file, index=False)
        logger.info(f"CSV saved with {len(available_columns)} columns to {output_file}")

    # def run_analysis(self) -> pd.DataFrame:
    #     logger.info("Starting Enhanced Medtronic ICD Analysis")
    #     # DON'T create all baselines upfront - do it on demand
    #     # self._create_all_patient_baselines()  # COMMENT THIS OUT
    #
    #     logger.info("Skipping upfront baseline creation - will create on demand")
    #
    ...
    #                 patient_episodes = self.patient_episodes.get(patient_id, {})
    #                 if patient_episodes:
    #                     rvshock = patient_episodes.get('RVshock', None)
    #                     bipecg = patient_episodes.get('BipECG', None)
    #
    #                     wavelet_lead = self._determine_best_wavelet_lead_for_patient(patient_episodes)
    #                     if wavelet_lead:
    ...
    #                         }
    #
    #             # Use analyse_label_group which adds to self.results
    ...
    def save_results(self, output_file: str):
        """Save results with selected columns"""
        if not self.results:
            logger.warning("No results to save")
            return
        ...
        # Define which columns you want and their order
        desired_columns = [
            # Identification
            'patient_id', 'label', 'group', 'primary_lead',
            'leads_used', 'signal_type', 'pacing_mode',
            ...
            # Zero-crossing metrics (simplified)
            'baseline_mean_zc_to_peak_ms', 'baseline_median_zc_to_peak_ms',
            'arrhythmia_mean_zc_to_peak_ms', 'arrhythmia_median_zc_to_peak_ms',
            'zc_timing_change_ms', 'relative_timing_change',
            'baseline_zc_time_ms', 'arrhythmia_zc_time_ms',
            'baseline_variability', 'arrhythmia_variability',
            'baseline_mean_ecg_zc_to_egm_peak_ms', 'arrhythmia_mean_ecg_zc_to_egm_peak_ms',
            'arrhythmia_median_ecg_zc_to_egm_peak_ms', 'baseline_median_ecg_zc_to_egm_peak_ms',
            ...
            # Signal quality
            'snr_db', 'snr_linear', 'signal_power','noise_power', 'rwave_amplitude'
            # Noise metrics
                                                                  'baseline_noise_level', 'baseline_snr_db',
            'arrhythmia_noise_level', 'arrhythmia_snr_db',

            # Laser noise metrics
            'baseline_laser1_noise', 'baseline_laser1_snr',
            'baseline_laser2_noise', 'baseline_laser2_snr',
            'arrhythmia_laser1_noise', 'arrhythmia_laser1_snr',
            'arrhythmia_laser2_noise', 'arrhythmia_laser2_snr',
        ]
...
def create_all_episode_plots(analyser, results_df: pd.DataFrame, output_dir: str):
    ...
    for idx, row in results_df.iterrows():
        ...
        if idx < len(analyser.results):
            ...
            if success:
                print(f"  ✓ Created plots for {episode_key}")
            else:
                print(f"  ✗ Failed to create plots for {episode_key}")
...
def create_visualizations_batch(analyser: MedtronicICDAnalyser,
    ...
    for idx, row in results_df.iterrows():
        ...
        try:
            ...
            if idx < len(analyser.results):
                ...
                if success:
                    successful_plots += 1
                    print(f"  ✓ Successfully created all plots")
                else:
                    failed_plots += 1
                    print(f"  ✗ Failed to create plots")

        except Exception as e:
            print(f"  ✗ Error processing {episode_key}: {e}")
            failed_plots += 1
            continue
    ...
    print(f"  Total episodes processed: {len(processed_episodes)}")
    print(f"  Successful: {successful_plots}")
    print(f"  Failed: {failed_plots}")
    print(f"  Output directory: {viz_path}")
    print("=" * 60)
...
def main():
    ...
    vt_dataset = [{
        ...
        'dataset_type': 'vt',
        'output_file': "vt_results_2025_11_16.csv",
        'viz_path': "./VT_3Plots/20251116"  # Updated path
    }]
    #
    ...
    #         'output_file': "inappropriate_haemodynamic_results_2025_11_16.csv",
    #         'viz_path': "./Inappropriate_3Plots/20251116"  # Updated path
    #     },
    #     {
    ...
    #         'dataset_type': 'inappropriate',
    #         'elevation_rate': 160,
    #         'output_file': "inappropriate_160bpm_results_2025_11_16.csv",
    #         'viz_path': "./Inappropriate_160bpm_3Plots/20251116"  # Updated path
    #     },
    #     {
    ...
    #         'dataset_type': 'inappropriate',
    #         'elevation_rate': 190,
    #         'output_file': "inappropriate_190bpm_results_2025_11_16.csv",
    #         'viz_path': "./Inappropriate_190bpm_3Plots/20251116"  # Updated path
    #     }]
    ...
    for dataset_config in all_datasets:
        print(f"\n{'=' * 80}")
        print(f"Processing Enhanced Dataset: {dataset_config['name']}")
        print(f"{'=' * 80}")
        ...
        try:
            ...
            analyser.metadata = analyser.metadata[analyser.metadata['Patient'] == 'A10']
            ...
            print(f"  Results saved to: {dataset_config['output_file']}")
            print(f"  Plots saved to: {dataset_config['viz_path']}")
    ...
    print("\n" + "=" * 80)
    print("ALL ENHANCED ANALYSES WITH 3 PLOTS PER EPISODE COMPLETE")
    print("=" * 80)
